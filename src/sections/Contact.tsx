...
import { CONTACT, PHONE_RAW, type ContactLocation } from '../content/contact'
...
export default function Contact() {
  const [mapLoc, setMapLoc] = useState<ContactLocation | null>(null)
  ...
  return (
    <section id="contact" className="relative bg-black px-4 py-20 md:px-6 md:py-28">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mt-10 grid grid-cols-1 gap-3 sm:gap-2 lg:grid-cols-2 lg:gap-1">
          {/* 左：联系方式 */}
          <CardShell index={0} className="flex flex-col bg-[#212121] p-5 sm:p-6 md:p-7">
          ...
          {/* 右：小地图 */}
          <CardShell index={1} className="flex flex-col bg-[#212121] p-5 sm:p-6 md:p-7">
            <div className="mb-4">
            ...
            <div className="mt-auto">
              <ContactMap loc={CONTACT.location} onClick={() => setMapLoc(CONTACT.location)} />
            </div>
          </CardShell>
        </div>
        ...
        <p className="mt-10 text-center text-[10px] text-gray-600 sm:text-xs">
          {CONTACT.studio} — {CONTACT.location.detail} · {CONTACT.email}
        </p>
      </div>

      <MapModal loc={mapLoc} onClose={() => setMapLoc(null)} />
    </section>
  )
}
