...
function ServiceCard({
  ...
  pkg: ServicePackage
  index: number
  onOpen: (pkg: ServicePackage) => void
}) {
  return (
    <CardShell index={index} className="bg-[#212121]">
      <button
        type="button"
        onClick={() => onOpen(pkg)}
        aria-label={`查看 ${pkg.name} 完整说明`}
        className="group flex h-full w-full flex-col p-5 text-left transition-colors duration-300 hover:bg-[#262626] focus:outline-none focus:ring-2 focus:ring-primary/50 sm:p-6"
      >
        ...
        {/* 点击进入详情的视觉提示 —— 移动端常显，桌面端 hover 浮起 */}
        <div className="mt-auto flex items-center justify-between pt-5 text-[11px] text-primary sm:text-xs">
          <span className="flex items-center gap-1.5 transition-opacity duration-300 group-hover:opacity-90">
            查看完整说明
            <ArrowRight
              className="h-3.5 w-3.5 -rotate-45 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
              strokeWidth={2.5}
            />
          </span>
          <span className="hidden text-[10px] text-gray-500 sm:inline">点击卡片</span>
        </div>
      </button>
    </CardShell>
  )
...
function PhotoPanel({ onOpen }: { onOpen: (pkg: ServicePackage) => void }) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:gap-2 md:grid-cols-2 md:gap-1">
      {PHOTO_PACKAGES.map((pkg, i) => (
        <ServiceCard key={pkg.id} pkg={pkg} index={i} onOpen={onOpen} />
      ))}
    </div>
  )
}

function VideoPanel({ onOpen }: { onOpen: (pkg: ServicePackage) => void }) {
  return (
...
function ComboPanel({ onOpen }: { onOpen: (pkg: ServicePackage) => void }) {
  return (
...
export default function Services() {
  ...
  const [openPkg, setOpenPkg] = useState<ServicePackage | null>(null)
    ...
    <section id="services" className="relative bg-black px-4 py-20 md:px-6 md:py-28">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mt-8 md:mt-10">
          {tab === 'photo' && <PhotoPanel key="photo" onOpen={setOpenPkg} />}
          {tab === 'video' && <VideoPanel key="video" onOpen={setOpenPkg} />}
          {tab === 'combo' && <ComboPanel key="combo" onOpen={setOpenPkg} />}
        </div>
      ...
      <ServiceDetailModal pkg={openPkg} onClose={() => setOpenPkg(null)} />
    </section>
  )
}